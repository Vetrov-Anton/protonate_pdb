"""Smoke tests: push the example fragment through the whole pipeline.

Run with: pytest -q   (needs an installed GROMACS to borrow a force field from)
"""
...
def test_propka_run_and_fixed_states(tmp_path):
    ...
    spec.residues = [
        ...
        ResidueSpec("A", 63, "", "HID"),      # histidine tautomer
...
def test_standard_pka_leaves_everything_default(tmp_path):
    ...
    states = _states(result)
    assert states[("A", 31)] == "ASH"
    # with standard pKa at pH 7 only histidines shift (to their neutral form)
...
def test_only_fixed_hydrogens(tmp_path):
    """Hydrogens stay only on the --fix residues; states are preserved."""
    ...
    # residue names (i.e. the states) stay in place
...
def test_force_field_is_never_touched(tmp_path):
    """TYR- does not build in the amber ports; either way the ff stays untouched."""
    ...
    try:
        ...
        # the expected refusal: the output directory is not even created
